...
- TypeScript: Browser cache handler now uses `cache: "no-store"` instead of `"no-cache"` — `no-cache` allowed stale `getObject` responses via 304 revalidation during the retry loop, causing all retries to fail with 412
...
- `set()` now uses the ETag from orphan repair instead of a separate `headObject` call, preventing stale-ETag race conditions caused by S3 eventual consistency
- `set()` now fails explicitly when orphan repair encounters an unexpected error, preventing creation of a second orphan
...
- CDK: Email-based access control for OIDC federation via `allowedEmails` on `OidcProvider`
  - Adds `StringEquals` condition on `${issuerUrl}:email` to the federated role trust policy
  - Validates that `allowedEmails` is non-empty when provided
  - Optional field; omitting it preserves existing audience-only behaviour
...
[0.1.30]: https://github.com/Portfoligno/immukv/releases/tag/0.1.30
