...
"""An async callable that returns S3Credentials.

Useful for STS temporary credentials that need periodic refresh.
Since boto3 does not natively support credential provider callbacks,
the provider is called once at client creation time. To refresh credentials,
recreate the ImmuKV client (it holds no irreplaceable state).
"""
